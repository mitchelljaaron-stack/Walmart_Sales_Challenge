  ...
  for(dept in unique(store_test$Dept)){
    ...
    dept_train <- dept_train %>%
      ...
      mutate(
        ...
        lag52 = lag(Weekly_Sales, 52)   # yearly pattern
      )
    dept_test <- dept_test %>%
      arrange(Date) %>%
      mutate(
    ...
    if(nrow(dept_train)==0){
      ...
      my_recipe <- recipe(Weekly_Sales ~ ., data = dept_train) %>%
        ...
        step_mutate(
          doy  = yday(Date),
          sin52 = sin(2*pi*yday(Date)/365),
          cos52 = cos(2*pi*yday(Date)/365)
        ) %>% 
        step_rm(Date, Store, Dept, IsHoliday) %>% 
        step_impute_mean(lag1, lag2, lag52)
...
vroom_write(x=all_preds, 
            file=paste0("./Predictions_final_a.csv"), delim=",")
